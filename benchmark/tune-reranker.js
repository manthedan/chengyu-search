...
function summarizeProfileReport(report, profile) {
    return {
        id: profile.id,
        label: profile.label,
        reranker_override: profile.override,
...
function runProfileEvaluation(profile, {
    ...
    const args = [
        EVALUATE_SCRIPT,
        '--variant-id', `reranker__${profile.id}`,
    ...
    if (result.status !== 0) {
        const error = new Error(`Reranker profile evaluation failed for ${profile.id}`);
...
async function main() {
    ...
    const profileIds = parseList(args.profiles, DEFAULT_RERANKER_PROFILES);
    const profiles = resolveRerankerProfiles(profileIds);
    ...
    for (const profile of profiles) {
        console.error(`\n=== Evaluating reranker profile ${profile.id} ===`);
    ...
    console.error('\nReranker profile ranking (higher is better, latency lower is better):');
    ...
    if (output) {
        console.error(`\nSaved full reranker report to ${output}`);
    }
}
